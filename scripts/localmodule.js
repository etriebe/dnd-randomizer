...
export class SFLocalHelpers {
  ...
  static async populateObjectsFromCompendiums(forceReload) {
    ...
    if (!this.dictionariesPopulated || forceReload) {
      ...
      if (FoundryUtils.getSystemId() === "pf2e") {
        this.calculateCreatureTraitsCount();
      }
  ...
  static calculateCreatureTraitsCount() {
    this.creatureTraitCount = {};
    const toLowerCase = true;
    const creatureTraitList = GeneralUtils.getUniqueValuesFromListOfArrays(this.allMonsters.map(i => i.traits), toLowerCase);

    for (let creatureTrait of creatureTraitList) {
      let monsterCount = this.allMonsters.filter(m => m.traits && m.traits.includes(creatureTrait.toLowerCase())).length;
      this.creatureTraitCount[creatureTrait] = monsterCount;
    }
  }
  ...
  static async filterMonstersFromCompendiums() {
    ...
    const savedTraitsSettings = game.settings.get(
      SFCONSTS.MODULE_NAME,
      "traitsToCreateEncountersFor"
    );

    // False is AND, True is OR mode
    const savedTraitsSelectionModeSettings = game.settings.get(
      SFCONSTS.MODULE_NAME,
      "traitsSelectionType"
    );
    ...
    const filteredTraitTypes = Array.from(Object.keys(SFLocalHelpers.creatureTraitCount)).filter((p) => {
      const el = savedTraitsSettings.find((i) => Object.keys(i)[0] == p);
      return !el || el[p] ? true : false;
    });
